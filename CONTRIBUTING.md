...
guaranteed, and none is promised anywhere on this repository.** One unpaid
person maintains it, around a day job.

A pull request that leaves the gates green is far more likely to be merged
than an issue is to be answered.
...
licence, while the copyright stays yours.
...
option would close on the first merge.

**For a substantial contribution, expect to be asked to say so in the pull
request itself**, in one line and in your own words.

**Nothing automated enforces it.** `convener-check-commits` reads every
commit in the range on every push, and most commits here are not written by
a person at all: the cockpit records a board vote through the GitHub API, a
scheduled job rewrites the decision register, another commits the published
projection of the data. And every instance duplicating this repository
inherits that check, so a rule about who may contribute code upstream would
...
- **One notion, one home.** `docs/engineering/content-rules.md` is the contract,
  ...
  rather than copied into the second. A copy is refused by a test.

## If you are forking rather than contributing

Name your own series. `TRADEMARK.md` says what the name and the mark
cover, and what the licence obliges a duplicate to keep. *Fork* there is
the free-software sense — your own product, out of this code — which the
licence allows and `README.md` distinguishes from GitHub's *Fork* button.
