...
1. Build the public form. Put the key in a file called `.env` at the
   ...
   of the run then succeeds or not. The form is created as a
...
The window is **twelve months** (decision G-14), set in `instance/data/config.yml`.
...
```
Board inactivity (G-14) - proposed, not applied; a human decides:
