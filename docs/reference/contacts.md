...
|---|---|
| Asking a contact at another institute to open the meeting room and start the recording | A meeting platform the organisation holds itself, with a manual fallback for the day nothing else works — see `docs/reference/operations.md` ("Meeting platform") |
| Sending the recording to someone else's channel to be uploaded | A video channel on the organisation's own account, which is also what makes the Board's publication gate real — see `docs/reference/operations.md` ("Video channel") |
