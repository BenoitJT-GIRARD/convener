...
| `declarations/` | product | The product's own three: `boundary.yml`, which names the paths the instance owns; `integrations.yml`, which names every external dependency the code knows about and what breaks without it; and `standing-up.yml`, the sequence that turns no repositories and no accounts into a running instance. |
