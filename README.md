...
| `tools/` | Every operational tool, whatever the language: the `convener-ops` package (data validation, integration status, the sweep, the public-data filter, certificate issuance and revocation, the retention sweep), the generators under `scripts/`, the one-shot migrations under `migrations/`, the Node rendering harness under `visuals/`, and the tests for all of them. |
