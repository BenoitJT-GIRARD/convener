...
def active_board(config: dict[str, Any], on: str) -> tuple[list[str], list[str]]:
    ...
    second one, and the annual meeting is what settles the question (G-14).
...
# gate (G-08) -- because the people bound by them are unpaid volunteers with
# day jobs, and a window that burns through a weekend has silently shortened
# itself. Others are calendar days by rule: the nomination window (G-05) and
