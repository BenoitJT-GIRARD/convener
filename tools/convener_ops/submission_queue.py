...
Phase 9, task 2. Until now every survey response a stranger submitted became
one `repository_dispatch`, and one billed GitHub Actions run, of its own --
the one line item in this project's budget whose volume is decided by people
the project has never met, and the one that grows when the series succeeds
(phase 9 spec, Sec 1). The submission now waits in this repository instead,
and one drain a day handles everything waiting in a single commit.
...
relay's request body byte for byte -- for the survey that is the hybrid
envelope `app/src/survey/encrypt.ts` produced, so the queue carries
ciphertext and an event id, and nothing else. That matters for retention:
D-22 destroys the *key*, not the records, so a residual entry in this
branch's history becomes unreadable at exactly the moment
`data/events/<id>/survey-responses.enc` does, by the same one operation.
A queue whose entries were plaintext would have no such property, which is
why `KINDS` is a closed set and not an open door.
...
-------------------------------
A survey response can only be decrypted with its own event's private key,
and a GitHub Actions job can only reach a secret through an expression
written in the workflow file itself -- never through a name a running step
computed. A drain covering N events therefore needs N such expressions
written out, so N has to be a constant. `MAX_EVENTS_PER_DRAIN` is that
constant, `tools/tests/test_submission_queue.py` pins the workflow to
exactly that many slots, and an event past the cap is deferred, never
dropped. Events are taken in order of their oldest waiting entry, so no
event can starve behind a busier one.
"""
...
from . import eventkeys, survey
from .registration import event_id_from_payload
...
#: The one kind of submission this queue carries today. A closed set, not a
...
#:
#: Registration is deliberately absent -- it must not be queued before the
#: distance-to-event delay of phase 9's task 3 exists, or a last-minute
#: registrant would wait a day for the room link that only the confirmation
#: e-mail carries. The proposal is absent for a different reason, recorded
#: in `docs/reference/operations.md`: `services/form-relay` receives Tally's
...
KINDS: Final = frozenset({SURVEY_KIND})
...
def plan_drain(
    ...
    *,
    ...
    begins with the millisecond the relay wrote them), and events by their
    oldest waiting entry, so a busy event can never starve a quiet one out
    of a slot. Two drains over the same queue produce the same plan.
    """
    refused: list[Note] = []
    deferred: list[Note] = []
    already: list[str] = []
    ...
    for name in sorted(entries):
        ...
        if isinstance(read, Note):
            refused.append(read)
            continue
        if not survey_open(read.event_id):
    ...
    chosen = list(by_event)[:max_events]
    for event_id in list(by_event)[max_events:]:
        for entry in by_event[event_id]:
            deferred.append(
                Note(
                    entry.name,
    ...
    handle = tuple(entry for event_id in chosen for entry in by_event[event_id])
    return DrainPlan(
        handle=handle,
...
def drain(
    ...
    for event_id in plan.event_ids:
        ...
        try:
            ...
            # A committed file this drain cannot parse: every entry for that
            # event waits rather than being written into a file whose shape
            # nobody understands. Deferred, not refused, for the same reason
            # a missing key is -- the operator can still fix the file.
            for entry in entries:
        ...
        for entry in entries:
            ...
            if response is None:
                # "could not be read", never "could not be decrypted":
                # `to_survey_response` folds a too-long answer into the same
                # uniform `None` as an undecryptable envelope, and this line
                # must not claim a cause it did not observe.
                refused.append(
                    Note(
                        entry.name,
                        f"a queued response for event {event_id} could not be read",
                    )
                )
                newly_handled.append(entry.name)
                continue
            current = survey.add_response(current, response, private_pem=private_pem)
            newly_handled.append(entry.name)
            handled += 1
    ...
    clear = tuple(sorted(set(newly_handled) | set(plan.already_handled)))
    return DrainOutcome(
        files=files,
