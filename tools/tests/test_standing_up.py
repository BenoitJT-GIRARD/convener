...
1. The committed page is byte-for-byte what the declaration derives, and so is
   the committed run sheet. That is the strongest of the four and it subsumes the
...
2. The page is read back and each declared step is found in it by number, by
   ...
   past.
3. The run sheet is read back the same way, and held to one thing more: it
   restates nothing. Every step's `does`, `check`, `degraded` and every line
   of its `walkthrough` is asserted *absent* from it, because the run sheet's
...
from generate_standing_up_run_sheet import (
    ACTION,
    ENTRY_POINT,
    RUN_SHEET_PATH,
    standing_up_run_sheet,
)
from generate_standing_up_run_sheet import COMMAND as RUN_SHEET_COMMAND
from generate_standing_up_run_sheet import main as run_sheet_main
...
def run_sheet() -> str:
    return (ROOT / RUN_SHEET_PATH).read_text(encoding="utf-8")
...
def test_every_step_states_the_actor_the_declaration_gives_it() -> None:
    """The field the later agent run sheet acts on.
...
def test_the_committed_run_sheet_is_what_the_declaration_derives() -> None:
    ...
    assert run_sheet() == standing_up_run_sheet(ROOT), (
        ...
        f"derives; run `{RUN_SHEET_COMMAND}` from `tools/`."
    )


def test_the_run_sheet_says_it_is_generated_and_names_what_derives_it() -> None:
    """A derived file that does not say so is one somebody will edit --
    and an agent is the reader most likely to edit a file it is following."""
    head = " ".join(run_sheet()[:1200].split())
    assert "generated" in head
    assert RUN_SHEET_COMMAND in head
    assert DECLARATION_PATH.as_posix() in head
...
def test_no_run_sheet_row_states_the_actor_the_declaration_did_not_give() -> None:
    ...
    rows = {
        line.split("`")[1]: line
        for line in run_sheet().splitlines()
...
def test_the_run_sheet_holds_no_step_the_declaration_does_not() -> None:
    ...
    """
    listed = re.findall(r"^\| (\d+) \| `([a-z_]+)` \|", run_sheet(), re.MULTILINE)
    sequence = declaration()
...
def test_the_run_sheet_restates_no_step_and_reads_the_declaration_instead() -> None:
...
def test_the_run_sheet_restates_no_stage_purpose_either() -> None:
...
def test_the_entry_point_names_every_rendering_and_restates_none() -> None:
    ...
    Nothing discovers the run sheet on its own now, so that file is the
    only route in: it has to name the run sheet, the guide and the declaration
    ...
    """
    text = entry_point()
    for named in (DECLARATION_PATH, DOC_PATH, RUN_SHEET_PATH):
...
def test_the_run_sheet_check_fails_when_it_has_never_been_written(
    fake_repo: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """A guard around something that does not exist has to fail loudly."""
    assert run_sheet_main(["--check"]) == 1
    assert not (fake_repo / RUN_SHEET_PATH).exists()
    err = capsys.readouterr().err
    assert RUN_SHEET_PATH.as_posix() in err
    assert RUN_SHEET_COMMAND in err


def test_the_run_sheet_check_leaves_a_stale_run_sheet_as_it_found_it(
    fake_repo: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    """A check that repairs is not a check."""
    written = fake_repo / RUN_SHEET_PATH
    ...
    assert run_sheet_main(["--check"]) == 1
    assert written.read_text(encoding="utf-8") == "stale\n"
    assert RUN_SHEET_COMMAND in capsys.readouterr().err


def test_writing_then_checking_the_run_sheet_passes(
    fake_repo: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    assert run_sheet_main([]) == 0
    assert run_sheet_main(["--check"]) == 0
    assert "matches the declaration" in capsys.readouterr().out


def test_a_second_run_sheet_write_changes_nothing_and_says_so(
    fake_repo: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    assert run_sheet_main([]) == 0
    capsys.readouterr()
...
def test_a_step_added_and_not_regenerated_makes_the_run_sheet_check_fail(
    fake_repo: Path,
) -> None:
...
def test_a_step_dropped_from_the_run_sheet_makes_the_run_sheet_check_fail(
    fake_repo: Path,
) -> None:
...
def test_an_actor_flipped_on_the_run_sheet_makes_the_run_sheet_check_fail(
    fake_repo: Path,
) -> None:
...
def test_a_title_reworded_on_the_run_sheet_makes_the_run_sheet_check_fail(
    fake_repo: Path,
) -> None:
...
def test_the_run_sheet_terminal_output_is_ascii(
    fake_repo: Path, capsys: pytest.CaptureFixture[str]
) -> None:
