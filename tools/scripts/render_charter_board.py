...
Four charters live under `brand/` and a duplicate chooses one of them by
...
copying `fonts/` beside itself. A poster's own `@font-face` asks for
...
What is on it
--------------
Per charter, read off `brand/` rather than listed (the same reading
...
def _font_data_uris(root: Path) -> dict[str, str]:
    ...
    return {
        ...
        for path in sorted((root / "fonts").glob("*.woff2"))
    }
...
def _charter_root(scratch: Path, root: Path, charter: Path) -> Path:
    ...
    shutil.copytree(root / "fonts", made / "fonts")
    return made
...
def render_board(root: Path) -> str:
    """The whole page, every charter under `brand/` in name order."""
...
<body>
  ...
  <code>brand/</code>, at every canvas <code>formats.FORMATS</code> names,
