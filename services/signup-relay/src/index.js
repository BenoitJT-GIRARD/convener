 ...
 * against the same `instance/keys/events/<id>.pub`, and the GitHub token is the
...
/**
 * Whether `instance/keys/events/<eventId>.pub` exists in the repository -- the
...
async function eventKeyExists(eventId, token, repository) {
  const url = contentsUrl(repository, `instance/keys/events/${encodeURIComponent(eventId)}.pub`);
 ...
 *
 * Reads `instance/public-data/survey-status.json` through the
 ...
 * `instance/keys/events/<id>.pub`, just a different path -- rather than a second
 ...
 * handler's (`tools/convener_ops/cli/journey/event.py::survey_enabled`, which reads
 * `instance/data/speakers.yml` directly) by construction, at the cost this
 ...
 *
 * `instance/public-data/survey-status.json` is committed on purpose (see
 * `.gitignore`'s own comment): a bare, sorted list of event ids, no
 * personal data, generated by
 * `tools/convener_ops/cli/publication.py::survey_status_public_data` and
 * pushed back to this repository by `deploy.yml`'s own
...
async function surveyEnabled(eventId, token, repository) {
  const url = contentsUrl(repository, 'instance/public-data/survey-status.json');
  const res = await fetch(url, {
    headers: {
      Authorization: `Bearer ${token}`,
      Accept: 'application/vnd.github+json',
