 ...
 * `examples/the-example-collective/` owns plus the product's two -- and more than one
 ...
 *  `examples/the-example-collective/instance/`. */
  ...
  if (!raw) {
    throw new Error(
      'VITE_EXAMPLE_SETTINGS is unset: this bundle was built without ' +
        ...
        'show (see examples/the-example-collective/instance/)',
    );
  }
...
function declaredString(key: string): string {
  ...
  if (typeof value !== 'string' || value === '') {
    throw new Error(
      `examples/the-example-collective/instance/config.json declares no ${key}, so the ` +
