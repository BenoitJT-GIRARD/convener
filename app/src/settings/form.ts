 ...
 *
 * - `alarm_after_hours` and the whole of `config/actions-budget.yml` are
 ...
 *   case;
 * - `config/queue-drain.yml`'s `max_silent_days` is read by *Retention
 *   liveness watchdog*, a different schedule;
 * - `queue_beyond_hours` reaches nothing at all until *Deploy app*
 *   regenerates and commits `public-data/registration-routing.json`, which
...
const DEPLOY: Effect = {
  when:
    ...
    'public-data/registration-routing.json, which is what the registration ' +
...
export const SETTINGS: readonly Setting[] = [
  ...
  },
  {
    ...
    what: 'How long data/queue-watch.yml may go without moving before the drain itself is called dead.',
    unit: 'days',
    kind: 'whole',
    effect: WATCHDOG,
    coupled: false,
  ...
  },
  {
    ...
    what: 'How long data/actions-usage.yml may go without moving before the measurement itself is called dead.',
    unit: 'days',
    kind: 'whole',
    effect: SWEEP,
    coupled: false,
...
export function whyNotAForm(path: string): string | null {
  ...
  if (path === 'public-data/') {
    return 'Derived from data/ by the product\'s own commands and committed by Deploy app. Typing into it would be typing an answer the next run overwrites.';
  }
  if (path === 'docs/governance/register.md') {
