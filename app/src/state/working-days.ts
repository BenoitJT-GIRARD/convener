 ...
 * publication objection gate (G-08) -- because the people bound by them are
 ...
 * nomination window (`board.ts::NOMINATION_WINDOW_DAYS`, G-05) and
 ...
 *
 * `tools/convener_ops/governance/rule.py` implements the same arithmetic for the
...
export function addWorkingDays(from: string, n: number): string {
  ...
  // `tools/convener_ops/governance/rule.py` answers `''` to the same input.
