...
describe('every top-level fixture key is read by somebody, in either language', () => {
  ...
  it('every block is read by at least one test, in either language', () => {
    ...
    // The whole of `app/tests/`, walked, and not this module's own
    // directory: the suite is one directory per domain of `app/src/`
    // since the sweep was written, and a reader anchored at
    // `__dirname` would have quietly narrowed to `state/`.
    const SUITE = resolve(__dirname, '..');
    const text = [
      corpus(toolsTests, '.py', true),
      corpus(SUITE, '.test.ts', true),
      corpus(SUITE, '.test.tsx', true),
    ].join('\n');
